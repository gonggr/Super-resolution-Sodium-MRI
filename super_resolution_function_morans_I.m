...
% -------------------------------------------------------------------------
% modifications: 2017/10 - guillaume.madelin@nyumc.org - change name for use in super_resolution processing
% -------------------------------------------------------------------------


function [M] = super_resolution_function_morans_I(grid,W,s)

    % ---- Check if standardising should be done
    if exist('s','var')
        if strcmp(s,'true'),  grid=zscore(grid);
    ...
    if ((mod(size(W,1),2)| mod(size(W,2),2))~=1), error('Weight matrix W needs to have uneven size (eg. 5x5)');  end
